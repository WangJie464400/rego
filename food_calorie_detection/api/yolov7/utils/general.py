...
def select_device(device='', batch_size=None):
    ...
    参数:
        device (str): 设备名称，如 'cpu' 或 '0,1,2,3'
        batch_size (int, 可选): 批量大小，用于确定是否使用CPU。

    返回:
        device (torch.device): 选择的设备。
    """
    ...
    if cuda:
        ...
        if c > 1 and batch_size:
            assert batch_size % c == 0, f'batch-size {batch_size} 不可被CUDA设备数量 {c} 整除'
